...
class Booking extends Component {
  ...
  cancelBook = () => {
    ...
    if (this.state.user.user.tourType === "regUser") {
      ...
      axios.delete(`http://localhost:7000/api/r-booking/delete/`+this.state.id)
        .then(response => {
          console.log(response);
        });
    }
    ...
    //tourGuy
      ...
      console.log(this.state.id+"this.state.idTour")
      axios.delete(`http://localhost:7000/api/t-booking/delete/`+this.state.id)
        .then(response => {
          console.log(response);
        });
    }
          ...
          {this.state.date.map((n, index) => (
            <div className="col mb-4">
              <div >
                ...
                <p>{this.state.id[index]}</p>
                <button onClick={() => this.cancelBook()}>Cancel this book</button>
              </div>
            </div>
          ))}
        </div>
